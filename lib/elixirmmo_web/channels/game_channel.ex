defmodule ElixirmmoWeb.GameChannel do
  ...
  def join("game", _params, socket) do
    ...
    Logger.info "Client joined: player_id=#{player_id} player=#{inspect player} user.x=#{user.x} user.y=#{user.y} user=#{inspect user}"
  ...
  def terminate(_, socket) do
    ...
    Logger.info "Client websocket terminated: player_id=#{player_id} player=#{inspect player}"
    user = Accounts.get_user_by_player_id(player_id)
